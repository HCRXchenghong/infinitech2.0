...
- 已新增管理端操作审计日志首版：关键后台写操作记录 actor、action、target、request_id、ip_hash、非敏感 payload 和 created_at，管理员可通过 `/api/admin/audit-logs` 查询，BFF 与 Admin Web 操作台已接入；PostgreSQL-backed Store 已把审计推进到规范化 `audit_logs` 表和 actor/action/target 查询索引，旧快照审计会幂等补入表。
