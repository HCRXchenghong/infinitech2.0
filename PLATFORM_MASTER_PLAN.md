...
- 已新增管理端操作审计日志首版：关键后台写操作记录 actor、action、target、request_id、ip_hash、非敏感 payload 和 created_at，管理员可通过 `/api/admin/audit-logs` 查询，BFF 与 Admin Web 操作台已接入。
...
- 管理端 P0 视图已完成运营快照绑定和操作审计首版，但仍需分页筛选、详情页、审核表单、审计检索页、敏感字段脱敏策略，商户端/骑手端剩余业务页面。
