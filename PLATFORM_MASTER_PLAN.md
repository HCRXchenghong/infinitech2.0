...
- 用户端第一阶段必须是原生微信小程序，继续使用 `WXML/WXSS/TypeScript`，不沿用旧版用户端跨端工程，也不改成 Flutter。
- 商户端和骑手端第一阶段分别使用独立 `Flutter/Dart` 工程。
- 管理端第一阶段为桌面 Web，加移动管理端 `Flutter/Dart`。
...
- 商户端：旧版 `merchant-app` 的经营、订单、商品、店铺、钱包页面可作为 Flutter 信息架构与视觉参考。
- 骑手端：旧版 `rider-app` 已有抢单大厅、任务、钱包、历史订单、接单设置等页面，可作为 Flutter 信息架构与视觉参考。
...
│   ├── merchant-flutter             # 商户端 Flutter/Dart
│   ├── rider-flutter                # 骑手端 Flutter/Dart
│   ├── admin-web                    # 桌面管理端 Web
│   └── admin-flutter                # 移动管理端 Flutter/Dart
├── services
│   ├── api-go                       # 核心业务 API
│   ├── bff                          # Web/原生微信小程序/Flutter 聚合层
...
- 面向原生微信小程序、Web 管理端和 Flutter 客户端的聚合层。
...
- 原生微信小程序/Flutter 客户端只请求后端拿 `walletRechargeUrl`。
...
- 骑手端 Flutter 已有抢单大厅首屏，可执行上线/离线、每日一次免责取消和拒绝当前派单。
...
- 已建立原生微信小程序、商户端 Flutter、骑手端 Flutter、管理端 Web、移动管理端 Flutter 的首层工程。
...
- 已新增 BFF 浏览器 CORS 白名单首版：默认允许本地管理端/Flutter 调试来源，支持 `BFF_ALLOWED_ORIGINS` 配置部署来源，并明确放行 `Authorization`、`Content-Type` 和 `X-Client-Kind` 请求头。
...
- 已实现商户端 Flutter 首批经营、订单、商品、团购核销和资质资料页面。
- 已实现骑手端 Flutter 首批抢单大厅和站长工作台页面，骑手可执行取货/送达、保证金缴纳/免押/退押申请，站长台可看骑手等级、接单耗时并配置任务，站长可创建本站骑手邀约，骑手/站长 API 工具已支持接受邀约带密码和账号密码登录。
