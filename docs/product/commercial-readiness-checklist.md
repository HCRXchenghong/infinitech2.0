...
- 本轮补充证据：已新增 `POST /api/admin/audit-logs/archive/request`，可按热存窗口生成审计冷归档 manifest、`sha256:v1` manifest hash、归档路径和 `audit.archive_requested` outbox 事件，并写入 `admin.audit_archive.requested` 审计；BFF、Admin Web、Store/HTTP 测试、架构守卫和 outbox-relay-worker 默认 topic 已覆盖。该能力是 WORM/冷归档请求首版，真实 WORM 对象存储写入、留存锁定和自动清理仍待补。
