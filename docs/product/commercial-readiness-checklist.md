...
- 本轮补充证据：已新增 `services/audit-archive-worker`，可领取 `audit.archive_requested` outbox、校验 manifest hash、上传 `application/x-ndjson` 归档、附带对象锁请求头和审计 hash 元数据，回写 `admin.audit_archive.completed` 完成证据，并按结果回写 outbox；Docker Compose 与 K8s 部署骨架已覆盖。仍需生产 bucket 强制对象锁、归档校验历史详情页/生产演练回查、保留期删除审批、KMS/链式不可抵赖签名和真实演练报告。
...
- 本轮补充证据：已新增 `GET /api/admin/audit-logs/archive/verifications`，可从 `admin.audit_archive.verified` 审计账本重建归档校验历史，支持 archive id、状态、时间范围和 limit 查询；BFF、Admin Web 操作目录、HTTP/Store/Admin/BFF 测试和架构守卫已覆盖。该能力是校验历史查询首版，完整详情页、生产演练报告和法律级不可抵赖归档仍待补。
