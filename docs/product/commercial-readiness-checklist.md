...
- 本轮补充证据：已新增 `services/audit-archive-worker`，可领取 `audit.archive_requested` outbox、校验 manifest hash、上传 `application/x-ndjson` 归档、附带对象锁请求头和审计 hash 元数据，并按结果回写 outbox；Docker Compose 与 K8s 部署骨架已覆盖。仍需生产 bucket 强制对象锁、归档回查、保留期删除审批、KMS/链式不可抵赖签名和真实演练报告。
