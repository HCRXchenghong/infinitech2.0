...
- 管理端 Web 最小运营控制台：
  ...
  - 已接 `/api/admin/rbac/policy`、`/api/admin/rbac/change-requests` 和 `/api/admin/rbac/change-requests/{id}/review` 权限治理入口
...
- 管理端审计账本：
  ...
  - 已落地 RBAC 权限治理查询、变更申请和审批/驳回审计首版，分权后台角色可读服务端真实矩阵，`admin`/`super_admin` 可提交待审批申请，另一名管理员可审批或驳回；申请与审批分别写入 `admin.rbac.change_requested` 和 `admin.rbac.change_reviewed` 审计，当前不自动改变运行时权限
