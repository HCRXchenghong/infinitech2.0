...
- 管理端 Web 最小运营控制台：
  ...
  - 已接 `/api/admin/rbac/policy`、`/api/admin/rbac/change-requests`、`/api/admin/rbac/change-requests/{id}/review` 和 `/api/admin/rbac/change-requests/{id}/apply` 权限治理入口
...
- 管理端审计账本：
  ...
  - 已落地 RBAC 权限治理查询、变更申请、审批/驳回和手动应用首版，分权后台角色可读服务端真实矩阵，`admin`/`super_admin` 可提交待审批申请，另一名管理员可审批或驳回；已审批申请可通过单独应用动作进入运行时权限矩阵，申请、审批和应用分别写入 `admin.rbac.change_requested`、`admin.rbac.change_reviewed` 和 `admin.rbac.change_applied` 审计
